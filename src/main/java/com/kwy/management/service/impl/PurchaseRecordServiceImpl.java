...
//        System.out.println(purchaseRecord.getId());
        Long goodsId = record.getId();
...
//        3. GoodsRecord 库存记录表插入数据
        ...
        goodsRecord.setOperateType(GoodsRecord.OPERATE_IN);
...
//              5.2 如果不存在 加上该种类 设置总量 最近入库 单价 估值
        ...
        materialOverviewlqw.eq(MaterialOverview::getCategory, record.getCategory());
        ...
        if (overview == null) {
            ...
            newOverview.setCategory(record.getCategory());
            newOverview.setNumber(record.getNumber());
            newOverview.setUpdateDate(record.getCreateDate());
            newOverview.setPrice(record.getPrice());
            newOverview.setValue(newOverview.getNumber() * newOverview.getPrice());
